/*
  # Esquema completo para HILOSdeCALIDAD - Sistema de Gestión de Ventas
...
CREATE POLICY "Permitir actualización de ventas" ON ventas
  FOR UPDATE TO authenticated
  USING (true);

-- Políticas de seguridad para ventas_detalle
CREATE POLICY "Permitir lectura de detalles de venta" ON ventas_detalle
  FOR SELECT TO authenticated
  USING (true);

CREATE POLICY "Permitir inserción de detalles de venta" ON ventas_detalle
  FOR INSERT TO authenticated
  WITH CHECK (true);

CREATE POLICY "Permitir actualización de detalles de venta" ON ventas_detalle
  FOR UPDATE TO authenticated
  USING (true);

-- Políticas de seguridad para eventos
CREATE POLICY "Permitir lectura de eventos" ON eventos
  FOR SELECT TO authenticated
  USING (true);

CREATE POLICY "Permitir inserción de eventos" ON eventos
  FOR INSERT TO authenticated
  WITH CHECK (true);
...
INSERT INTO usuarios (nombre, telefono, dni) VALUES
  ('Juan Pérez García', '987654321', '12345678'),
  ('María González López', '876543210', '87654321'),
  ('Carlos Rodríguez Silva', '765432109', '23456789'),
  ('Ana Martínez Torres', '654321098', '34567890'),
  ('Luis Fernández Ruiz', '543210987', '45678901')
...
INSERT INTO eventos (tipo, descripcion, usuario) VALUES
  ('Sistema', 'Base de datos inicializada correctamente', 'Sistema'),
